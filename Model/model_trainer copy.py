...
class DataPrefetcher:
    def __init__(self, loader, device):
        self.reward_objects = loader.dataset.dataset.reward_objects
        print(loader.dataset.dataset.reward_objects.name)
...
def train(config, train_data_loader, val_data_loader, model, dicts, info_dicts):
    ...
    optimizer = GrokFastAdamW(model.parameters(), lr=config['training']['learning_rate'], weight_decay=0.0001, normalize_lr=False) #GrokFastAdamW(model.parameters(), lr=config['training']['learning_rate']) #, weight_decay=0.1)
    ...
    for epoch in range(start_epoch, config['training']['num_epochs']):
        ...
        data_loader.epoch += 1 remember to iterate /increase the epoch in the data loader.
...
class CumulativeReturnBatch:
    ...
    def add_pct_change(self, pct_changes):
        """
        Updates the cumulative product with a batch of percentage changes for active items only.
        
        Parameters:
        pct_changes (list, numpy array, or torch tensor): A batch of percentage changes.
        """
        # Convert pct_changes to a tensor if it's not already one
        if not isinstance(pct_changes, torch.Tensor):
            pct_changes = torch.tensor(pct_changes, device=self.device, dtype=torch.float32)
        else:
            pct_changes = pct_changes.to(self.device, dtype=torch.float32)
        ...
        if active_indices.numel() > 0:
            self.cumulative_product[active_indices] *= (1 + pct_changes[active_indices] / 100)
    ...
    def should_halt_all_outputs(self, percentage=0.5):
        """
        ...
        """
        under_threshold = self.is_under_threshold(self.threshold)
...
def epoch_loop(model, data_loader, optimizer, device, timestamp, epoch, epsilon, replay_buffer, is_training=True, scheduler=None, scaler=None):
    ...
    nan_detected = False
    accumulation_steps = max(len(data_loader) / config['training']['accumulation_per_epoch'], 1)
    
    # Variables for tracking profits
    cumulative_profit = []
    last_batch_pct_change = 0.0
    ...
    if is_training:
        ...
        # index_range = range(batch_length * BATCH_SIZE)
    else:
        ...
        # index_range = range(len(data_loader))
    ...
    # batch_loss           = torch.zeros(BATCH_SIZE, dtype=torch.float16, device=device)
    # total_loss           = torch.zeros(BATCH_SIZE, dtype=torch.float16, device=device)
    # effective_batch_loss = torch.zeros(BATCH_SIZE, dtype=torch.float16, device=device)
    # epoch_loss           = torch.zeros(BATCH_SIZE, dtype=torch.float16, device=device)
    ...
    while batch is not None:
        ...
        # done = cum_return.get_done_tensor() #Need to maybe change this to get a bool if too many negative values past the threshold, so we're done. (If we kill a batch because we're done, how dow we not screw up with all the continueing on batches, seems messy... Maybe if one in the batch is done, will kill training and worry about a work around for that at a later time. But yeah, done if at the end of the len of data_loader or whatever too.)
        # if index == len(data_loader) - 1 or cum_return.update_done_tensor(index)#is_under_threshold(-0.95):
        #     done = True
        # else:
        #     done = False
        ...
        # current_effective_leverage = torch.zeros(BATCH_SIZE, dtype=torch.float16, device=device)# 0.0
        # if current_trades is not None: do this torch bitwise or whatever
        #     current_trade.update_hold_time()
        #     current_effective_leverage, current_position = current_trade.calculate_effective_leverage(current_prices)
        # else:
        #     current_position = 2  # No position
            
        # return current_position through batch['current_positions']. We will get the current position during the prefetch of the reward object.
        
        
        # lowest_tf_next = batch['lowest_tf_next_n_opens'][0]#.values
        # goal_tf_next = batch['next_n_opens'][0]#.values
        
        # # move hour_top_or_bot to the prefetcher to make this shit faster/asyncrounous
        ...
        #     thirty_min_price = current_prices
        # elif not hour_top_or_bot and thirty_min_price != 0:
        #     goal_tf_next = np.insert(goal_tf_next, 0, thirty_min_price)
        
        # reward_object.next(current_trade, lowest_tf_open_array=lowest_tf_next, goal_tf_open_array=goal_tf_next, target_tf_open=current_prices, index=index)
        
        if epoch == 0:
            # move this to the prefetcher, if possible
            actions = batch['ideal_action'].to(device)# prefetcher.reward_objects[idx].get_ideal_action()#current_trade, lowest_tf_next, goal_tf_next, batch['raw_data'][0]['open'])
        else:
            if random.random() < epsilon and is_training:
                # Explore: select a random action
                # action = random.randrange(model.num_actions) need to get action as shape [batch_size, 1]
                ...
                # positions, leverages, actions = zip(*history_items)  # Unpack history into separate lists
                
                # turn action_leverage_history values into tensor, feed the tensor into process_batch for model input 
                # Convert lists to tensors
                # positions_tensor = torch.tensor(positions, dtype=torch.long, device=device).unsqueeze(0)  # Shape: [1, sequence_length]
                # leverages_tensor = torch.tensor(leverages, dtype=torch.float32, device=device).unsqueeze(0)  # Shape: [1, sequence_length]
                # actions_tensor = torch.tensor(actions, dtype=torch.long, device=device).unsqueeze(0)  # Shape: [1, sequence_length]
                ...
                # Exploit: select the best action
                actions = q_values.max(1)[1] #.item() # what does max(1) do? # torch.max(q_values, 1)[1].view(1, 1).item()
        # I THINK THIS HAS TO GET MOVED UP TO THE ELSE STATEMENT ABOVE
        # Convert action_leverage_history into tensors
        # history_items = action_leverage_history.get_items()
        # positions, leverages, actions = zip(*history_items)  # Unpack history into separate lists
            
        # ~~~~ SOMETHING LIKE THIS ~~~~~~~
        # """Basically, GPT, we need to get the rewards, new_current_trade, profit, hold_time, end_tick_effective_leverage, pct_change values from the prefetcher.reward_objects[reward_object_idx].load_future_state_and_get_environment_step(action) this fn is as such:    def load_future_state_and_get_environment_step(self, action):
        # future_state = self.future_states[action]
        # self.__dict__.update(future_state.__dict__)
        # return self.environment_step 
        
        # It would be nice if the retreival of self.environment_step could be done an all locations/values of reward_object_idx like a vecotrized operation. Otherwise, doing it with a threadpool or process pool could be cool too. Would be nice if we kept the instantication of the threads before the for loop and then just called the threads in the for loop. Would be nice if we could do this with a single line of code. <--that last autofill sentence tho... lol """
        ...
        #     action=action #current_trade, action, current_price, index, batch['next_n_opens'], batch['volatility'], nothing_hold_time
        # )
        # for reward_object in reward_objects:
        #     rewards, new_current_trade, profit, hold_time, end_tick_effective_leverage, pct_change = reward_object.load_future_state_and_get_environment_step()
        
        # results = []
        # # HEY HEY HEY -Try a threadpool or multiprocessing pool to see the speed difference
        # for reward_object_idx, action in zip(range(BATCH_SIZE), actions):
        #     results.append(prefetcher.reward_objects[reward_object_idx].load_future_state_and_get_environment_step(action))
        
        # for result in results:
        #     #need to apply this crap to a list of results, use zip to unpack results?
        #     rewards, new_current_trade, profit, hold_time, end_tick_effective_leverage, pct_change = result
        # ~~~~~~ /SOMETHING LIKE THIS ~~~~~~~
        
                # Simulate environment response for each batch item
        # for i in range(BATCH_SIZE):
        
        # We could prob combine these two for loops into one. We should make sure that the prefetcher.reward_objects[reward_object_idx].load_future_state_and_get_environment_step(action) is vectorized across the batch_size dimension.
        # Simulate environment response for each batch item
        results = []
        for i in range(BATCH_SIZE):
            action = actions[i].item()
            result = reward_objects[i].load_future_state_and_get_environment_step(action)
            results.append(result)
        
        # Unpack results
        rewards_list, new_current_trades, profits, hold_times, end_tick_effective_leverages, pct_changes = zip(*results)
        rewards = torch.tensor(rewards_list, dtype=torch.float32, device=device)  # Shape: [batch_size]
        epoch_reward += rewards.sum().item()
        
        # Update current trades
        for i in range(BATCH_SIZE):
            if current_trades[i] != new_current_trades[i]:
                if current_trades[i] is not None and current_trades[i].exit_index is not None:
                    # Trade was closed
                    profit_pct = current_trades[i].profit_pct
                    ...
                    cum_return.add_pct_change(profit_pct)
                    cumulative_profit.append(profit_pct)
                    last_batch_pct_change[i] = profit_pct
                current_trades[i] = new_current_trades[i]
        
        
        batch = prefetcher.next()
        
        # """Dear Sir or Maddam GPT, We need to add the items here. Please do so for further processing as we've seen above. Thank you."""
        # action_leverage_history.add_item((current_position "SEE current_possitions above (note the plurality)", end_tick_effective_leverage, action))
        ...
        # """Dear GPT, maybe reward can equal rewards[:, actions] (Need to make the actions a tensor)"""
        # reward = rewards[action]
        # epoch_reward += reward
        
        # """Dear GPT, this portion thru to and including replay_buffer.add() needs to also allow for BATCH_SIZE dim items, whereas not it is a list of (implicitly,) batch_size=1. Just you made the action_leverage_history portion above now work for batch_size rather than just 1. Thank you."""
        # """Given our use of the replay buffer below, we should probably ensure that each addition to the replay buffer in the batch_size dimension is its own, unique addition. Or, more to the point, the states can be ran mutually exclusively from one another, ie we aren't loading batches determinisitcally/ all batch_size objects at the same time. Hope you understand what I mean. """
        # # Get the updated history for next_state
        # next_history_items = action_leverage_history.get_items()
        # next_positions, next_leverages, next_actions = zip(*next_history_items)
        # next_positions = list(next_positions)
        # next_leverages = list(next_leverages)
        # next_actions = list(next_actions)
        
        # # Store experience in replay buffer
        # replay_buffer.add(
        #                 state = (state.cpu().numpy(), list(positions), list(leverages), list(actions)),
        #                 action = action,
        #                 reward = reward,
        #                 next_state = (next_state.cpu().numpy(), list(next_positions), list(next_leverages), list(next_actions)),
        #                 done = done
        #                 )
        ...
        # Store experience in replay buffer
        replay_buffer.add(
            state=(state.cpu().numpy(), positions_tensor.cpu().numpy(), leverages_tensor.cpu().numpy(), actions_tensor.cpu().numpy()),
            action=actions.cpu().numpy(),
            reward=rewards.cpu().numpy(),
            next_state=(next_state.cpu().numpy(), next_positions_tensor.cpu().numpy(), next_leverages_tensor.cpu().numpy(), next_actions_tensor.cpu().numpy()),
            done=done.cpu().numpy()
        )
        

        # """Need to make this absolute chunk of code below work for arrays of current_trade that are size batch_size, need to find a way to handle the lists too. They don't necessarily need to be lists, but they do need to be the same size as the batch_size. """
        ...
        #         cum_return.add_pct_change(current_trade.profit_pct)
        ...
        #     """NOTE: MR./MS. GPT, this if statment above used to contain a for loop for running multiple mini-batches to accumulate the loss over more data, this was done before the initial stuff above for getting the actions of multiple back-simulations. Just something for you to know I guess. There may be some vestiges of this in the code below that may be optimized-away/removed."""
        ...
        #     actions_ = torch.tensor(actions, dtype=torch.long, device=device)
        #     rewards_ = torch.tensor(rewards, dtype=torch.float32, device=device)
        #     dones_ = torch.tensor(dones, dtype=torch.float32, device=device)
        ...
        #     # Calculate the indices for the mini-batch
        #     # start_idx = i * BATCH_SIZE
        #     # end_idx = start_idx + BATCH_SIZE

        #     # Extract the mini-batch from your data
        #     state_batch = state_tensors.squeeze(1)#[start_idx:end_idx].squeeze(1)
        #     position_batch = state_positions#[start_idx:end_idx]
        #     effective_leverage_batch = state_effective_leverages#[start_idx:end_idx]
        #     actions_state_batch = state_actions#[start_idx:end_idx]

        #     next_state_batch = next_state_tensors.squeeze(1)#[start_idx:end_idx].squeeze(1)
        #     next_position_batch = next_state_positions#[start_idx:end_idx]
        #     next_effective_leverage_batch = next_state_effective_leverages#[start_idx:end_idx]
        #     next_actions_state_batch = next_state_actions#[start_idx:end_idx]

        #     actions_batch = actions_#[start_idx:end_idx]
        #     rewards_batch = rewards_#[start_idx:end_idx]
        #     dones_batch = dones_#[start_idx:end_idx]
        ...
        #                 effective_leverages = effective_leverage_batch,
        #                 actions = actions_state_batch
        #             ).gather(1, actions_batch.unsqueeze(1))
            
        #         with torch.no_grad():
        #             # Get target Q values
        #             max_next_q_values = target_model(
        #                 tensors = next_state_batch,
        #                 positions = next_position_batch,
        #                 effective_leverages = next_effective_leverage_batch,
        #                 actions = next_actions_state_batch
        #             ).max(1)[0].unsqueeze(1)
        #             target_q_values = rewards_batch.unsqueeze(1) + (gamma * max_next_q_values * (1 - dones_batch.unsqueeze(1)))
        #     all_current_q_values = current_q_values
        #     all_target_q_values = target_q_values.to(torch.float16)
        #     # all_current_q_values.append(current_q_values)
        #     # all_target_q_values.append(target_q_values.to(torch.float16))
            
        #     # Concatenate all mini-batch results
        #     # all_current_q_values = torch.cat(all_current_q_values, dim=0)
        #     # all_target_q_values = torch.cat(all_target_q_values, dim=0)
            
            
        #     batch_loss += criterion(all_current_q_values, all_target_q_values)
        ...
        #     if check_nan(next(model.parameters()).data, "model parameters"):
        #         logging.info("NaN detected in model parameters")
        #         nan_detected = True
        #         break
            
        #     if index % MODEL_UPDATE_FREQ == 0 and is_training: # MODEL_UPDATE_FREQ divided by num_batches?
        ...
        # timestamps.append(raw_data[0].name)  # Replace with actual timestamp if available
        ...
        cumulative_profit_over_time.append(cum_return.get_cumulative_return().cpu().numpy())
        open_prices.extend([rd['open'] for rd in raw_data])
        
        # if len(cumulative_profit) > 0:
        #     compound_return = calculate_cumulative_return_list(cumulative_profit) """ This should be compout_return = cum_return.get_cumulative_return()"""
        ...
        # open_prices.append(raw_data[0]['open'])  # Assuming raw_data[0] corresponds to current time step
        # print(np.round(rewards, 4))
        # boberts = np.round(rewards, 4)
        # # Collect results for logging
        # result_entry = {
        #     'index': index,
        #     'timestamp': raw_data[0].name, #batch.get('timestamp', index),  # Replace with actual timestamp if available
        #     'action': action,
        #     'cumulative_profit': compound_return,
        #     'reward': np.round(rewards[action], 4), 
        #     'rewards': boberts, #np.round(rewards, 4),
        #     'current_price': current_prices,
        #     'next_prices': batch['next_n_opens'],
        #     'lowest_tf_next_prices': batch['lowest_tf_next_n_opens'],
        #     'current_position': current_position, """This among other values here may need to be updated to be a list or array of values rather than a single value"""
        #     'profit': profit, 
        #     'hold_time': hold_time,
        #     'effective_leverage': end_tick_effective_leverage, #current_trade.calculate_effective_leverage(current_price) if current_trade is not None else 0.0, #portfolio.effective_leverage,
        ...
        for i in range(BATCH_SIZE):
            result_entry = {
                ...
                'action': actions[i].item(),
                'cumulative_profit': cumulative_profit_over_time[-1][i],
        ...
        if (index + 1) % accumulation_steps == 0 or index == 0 or cum_return.all():# """Need to fix references of done, in this case we want the final print in the event that being done haults further processing. NOTE: see 'if done:/n break' below."""
            current_time = time.time()
            elapsed_time = current_time - loop_start
            time_per_index = elapsed_time / (index + 1)
            progress_percent = ((index + 1) / len(data_loader)) * 100
            remaining_iterations = len(data_loader) - (index + 1)
            estimated_remaining_time = remaining_iterations * time_per_index
            
            # if len(cumulative_profit) > 0:
            #     compound_return = calculate_cumulative_return_list(cumulative_profit)
            #     cum_return = sum(cumulative_profit) / len(cumulative_profit) #calculate_cumulative_return_list(cumulative_profit)
            #     # avg_profit = cum_return / (index + 1)
            # else:
            #     cum_return = 0
            #     compound_return = 0
                # avg_profit = 0
            ...
            logging.info(f"CPU Memory: {psutil.virtual_memory().used / 1e9:.2f} GB")
            ...
            logging.info(f"Avg Batch Losses: {effective_batch_loss.item():.9f}FIXME")
            logging.info(f"Last Avg Loss: {batch_loss.item():.9f}")
            logging.info(f"Last Profit: BATCH | AVG | COMPOUND --- {last_batch_pct_change:.6f} | {cum_return:.6f} | {cum_return.get_cumulative_return():.6f}")
            ...
            effective_batch_loss = torch.tensor(0.0, dtype=torch.float16, device=device)
    ...
    return epoch_loss, nan_detected, epoch_reward/index
...
if __name__ == '__main__':
    try: #to continue, implement 2 timeframe model. Implement float16 for mixed precision training, but use float32 for loss, and implement time/position embedding
        ...
        train_data_loader, val_data_loader = get_data_loaders(data, config, info_dicts['start_epoch'])
