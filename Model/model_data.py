...
class TimeSeriesDataset(torch.utils.data.Dataset):
    def __init__(self, data, sequence_length, num_outputs): #, epoch, valid_indices, data_type):       
        ...
        self.valid_indices = self.get_valid_indices(len(self.data.df_dict[data.shortest_timeframe]))
        
        # Suppose you have num_sequences sequences
        self.batch_size = config['training']['batch_size']
        # self.reward_objects = [Reward_Simple() for _ in range(self.batch_size)] #this never gets reset after each epoch
        # self.tf_overlap_price = [0 for _ in range(self.batch_size)]
        # self.epoch = epoch

    # def reset_reward_objects(self):
    ...
    def get_valid_indices(self, valid_indices):
        ...
        last_index = (valid_indices - self.data.future_ticks ) - ((valid_indices - self.data.future_ticks -1) % config['training']['batch_size']) - 1 # This -1 is important, idk... but it is.
    ...
    def __getitem__(self, idx):
        try:
            ...
            return {
                    ...
                    'raw_data': raw_data,
                    'next_input': torch.FloatTensor(next_processed_data),
                    'next_n_opens': next_n_opens,
                    'lowest_tf_next_n_opens': lowest_tf_next_n_opens,
                    'lowest_tf_next_n_opens_minute': lowest_tf_next_n_opens.index[0].to_pydatetime().minute,
...
def get_data_loaders(data, config, epoch):
    ...
    train_size = int(config['training']['train_val_split'] * total_samples)
    ...
    # try changing ParallelSequenceSampler to the default SequentialSampler. The problem of 2 simulations is because the parallelsequencesampler is returning 2 runs, but the parallel process wants to get num_worker num of processes, so it runs Num_workers * num of runs according to the sampler. go with a sampler that provides 1 at a time!
    ...
    val_sampler = SequentialSampler(val_dataset) #, batch_size=1)  # For validation, you might use batch_size=1
        ...
        #config['training']['num_workers'], #It seems that having a worker causes some issue with creating all 4 self.future_states. I think it may be because the worker is trying to access the same data as the main process
    ...
    # train_loader = DataLoader(
    #     train_dataset,
    #     # prefetch_factor=1, #It seems that having a worker causes some issue with creating all 4 self.future_states. I think it may be because the worker is trying to access the same data as the main process
    #     # For debugging this odd issue, see DataPrefetcher preload()
    #     # IF I'M UNABLE TO FIX THIS I NEED TO STOP THE FOR ACTION IN RANGE(4) LOOP! and instead just run/call the simulate_environment_step from the main loop.
